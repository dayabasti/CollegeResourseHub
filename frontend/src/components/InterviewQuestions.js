...
function Interview({ role }) {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}/api/resources/section/interview`)
      .then((res) => {
        const sorted = sortResources(res.data, sortOption);
        setResources(res.data);
        setFiltered(sorted);
      })
      .catch((err) => console.error("Error fetching interview resources:", err));
  }, [sortOption]);
  ...
  const handleView = async (id) => {
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/api/resources/section/interview`);
      setResources(res.data);
  ...
  const handleRate = async (id, rating) => {
    try {
      ...
      const updated = await axios.get(`${API_BASE_URL}/api/resources/section/interview`);
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-indigo-100 to-pink-100 min-h-screen">
      <h2 className="text-3xl font-extrabold text-center mb-6 text-pink-700">🧠 Interview Questions</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <input
          ...
          placeholder="Search interviews by title or tags..."
          className="p-3 border border-gray-300 rounded-lg w-full sm:w-2/3 focus:outline-none focus:ring-2 focus:ring-pink-400"
        />
        <select
          value={sortOption}
          onChange={(e) => setSortOption(e.target.value)}
          className="p-3 border border-gray-300 rounded-lg w-full sm:w-1/3 focus:outline-none focus:ring-2 focus:ring-pink-400"
        >
        ...
        {filtered.map((res) => (
          <div key={res._id} className="bg-white/60 backdrop-blur rounded-lg p-4 shadow hover:shadow-md transition-all duration-300 text-sm space-y-2">
            ...
            <a
              ...
              className="text-pink-700 hover:underline font-semibold block truncate"
            >
              {res.title}
            </a>
