...
function PYQs({ role }) {
  ...
  useEffect(() => {
    axios
      .get(`${API}/api/resources/section/pyqs`)
      .then((res) => {
  ...
  const handleView = async (id) => {
    try {
      ...
      const res = await axios.get(`${API}/api/resources/section/pyqs`);
      setResources(res.data);
  ...
  const handleRate = async (id, rating) => {
    try {
      ...
      const updated = await axios.get(`${API}/api/resources/section/pyqs`);
